...
5. `Tripo Rifles README` - 11 edges
6. `MakeSlider()` - 9 edges
7. `FLinearColor` - 9 edges
...
- `Rifle Icon Mk1` --references--> `Satisfactory Laser Rifle Guide Part 0`  [INFERRED]
  Content/Equipment/LaserRifle/Icons/src/rifle_icon_Mk1.png → satisfactory-laser-rifle-guide.md
...
Cohesion: 0.08
Nodes (25): Mk01 Color, Mk01 Metallic, Mk01 Normal, Mk01 Roughness, Mk01 Tex Base Color, Mk01 Tex Metallic, Mk01 Tex Normal, Mk01 Tex Roughness (+17 more)
...
## Knowledge Gaps
- **71 isolated node(s):** `int32`, `FPaintArgs`, `FGeometry`, `FSlateRect`, `FSlateWindowElementList` (+66 more)
...
- **Are the 9 inferred relationships involving `Tripo Rifles README` (e.g. with `Mk01 Color` and `Mk01 Metallic`) actually correct?**
  _`Tripo Rifles README` has 9 INFERRED edges - model-reasoned connections that need verification._
- **What connects `# IMPORTANT: set CDO defaults then SAVE (no post-set recompile -- recompiling a`, `columns = list of columns; each column = ordered list of schematic class     nam`, `int32` to the rest of the system?**
